...
class PaperService:
                ...
                try:
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            # 仅针对特定的 5xx 错误或网络错误进行重试
                            status_code = None
                            if isinstance(e, httpx.HTTPStatusError):
                                status_code = e.response.status_code
                            
                            # 如果是状态码错误，仅重试 502/503/504；如果是网络请求错误（如超时、断连），则也尝试重试
                            is_retryable = (status_code in [502, 503, 504]) or isinstance(e, httpx.RequestError)
                            
                            if attempt < max_retries - 1 and is_retryable:
                                wait_time = 2 ** attempt # 指数退避: 1s, 2s
                                logger.warning(f"下载失败 ({e})，将在 {wait_time}s 后进行第 {attempt+2} 次重试... (URL: {url})")
                                await asyncio.sleep(wait_time)
                            else:
                                raise e # 已达到最大重试次数或非可重试错误
                    
                    if not resp:
                        raise ValueError("下载论文失败，重试后仍未成功")
                    ...
                    upload_resp = await self.upload_paper(
                        ...
                        source_ref=req.source_id or req.arxiv_id
                    )
    ...
    async def upload_paper(
        ...
        # 1. 验证文件
        if not self._validate_file(filename, file_content, source=source):
        ...
        try:
            ...
            extracted_title = title or safe_filename
            extracted_authors = authors or []
            ...
            try:
                ...
                # 只有当元数据未提供时，才尝试从PDF提取
                # 注意：即使提取失败也继续，因为我们已经有文件名作为兜底标题
                ...
                if not summary and metadata.get("abstract"):
                    extracted_summary = metadata["abstract"]
                if source == "upload" and metadata.get("source"):
                    extracted_source = metadata["source"]
                
                # source_ref 优先级：参数 > 解析器提取
                extracted_source_ref = source_ref or metadata.get("source_id")
                
                # 处理日期 (只有在未提供 published_at 时才从 PDF 提取)
                if not published_at and metadata.get("published_date"):
                    try:
                ...
                if extracted_source == "arXiv" and extracted_source_ref:
                    try:
                        ...
                        if arxiv_meta:
                            if not title and arxiv_meta.get("title"):
                                extracted_title = arxiv_meta["title"]
                            if not authors and arxiv_meta.get("authors"):
                                extracted_authors = arxiv_meta["authors"]
                            if not summary and arxiv_meta.get("summary"):
                                extracted_summary = arxiv_meta["summary"]
                            if not published_at and arxiv_meta.get("published_at"):
            ...
            return PaperUploadResponse(
                ...
                status=paper.analysis_status,  # 使用原始枚举字符串
                title=paper.title,
    ...
    def _validate_file(self, filename: str, file_content: bytes, source: str = "upload") -> bool:
        """
        验证文件类型和大小
        """
        # 检查文件大小
        ...
        if not file_content.startswith(b'%PDF'):
            if source == "upload":
                ...
                logger.warning(f"从网络下载的内容不是有效的PDF (type: {file_ext})，将继续创建记录但解析可能会失败")
                # 对于网络源，即使不是PDF也允许创建记录，以便用户能看到失败原因而非静默失败
                return True
    ...
    async def _create_paper_record(
        ...
        """
        # 处理发布日期 (Published At) - 强制转换为 offset-naive 格式以匹配数据库配置 (TIMESTAMP WITHOUT TIME ZONE)
    ...
    async def get_paper_status(self, paper_id: UUID, user_id: UUID) -> Optional[PaperDTO]:
        ...
        # 2. 获取最相关的主任务 Job 信息 (process_pdf 或 vectorize)
        job = await JobRepository.get_primary_job_by_paper_id(self.session, paper_id)
        if job:
        ...
        if not active_parse_job and not has_completed_parse_job and dto.analysis_status != PaperStatus.COMPLETED:
    ...
    async def get_user_papers(
        ...
        for p in papers:
            ...
            # 如果是处理中或失败，获取最相关的主任务 Job 信息
            if dto.analysis_status in [PaperStatus.PROCESSING, PaperStatus.PENDING, PaperStatus.PARSING, PaperStatus.FAILED]:
                job = await JobRepository.get_primary_job_by_paper_id(self.session, p.id)
                if job:
                    dto.job_id = str(job.id)
                    dto.latest_job_type = job.type
            dtos.append(dto)
        return dtos
...
class PaperProcessingService:
    ...
    async def parse_text(self, paper_id: UUID, job_id: UUID, redis: Any = None) -> bool:
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                # 防止重复解析: 如果已解析/处理完成，直接跳过
                if paper.analysis_status in [PaperStatus.PARSED, PaperStatus.PROCESSING, PaperStatus.COMPLETED]:
                    logger.info(f"Paper {paper_id} already in status {paper.analysis_status}, skipping re-parse")
                    await _update_progress("finished", 100.0, "succeeded")
                    # 仍然尝试触发后续任务（去重逻辑会防止重复创建）
                    await self._trigger_next_tasks(paper_id, paper.user_id)
                    return True
                
                # 更新论文状态为 parsing
                await PaperRepository.update_paper_status(session, paper_id, PaperStatus.PARSING)
            ...
            text_content = await self._parse_pdf(file_path, parser_type=settings.pdf_parser_type)
            if not text_content:
            ...
            if isinstance(title, str): title = title.strip().replace("\x00", "")
            
            summary = metadata.get("abstract")
            if isinstance(summary, str): summary = summary.strip().replace("\x00", "")
            ...
            async with async_session_factory() as session:
                # 重新获取最新的 Paper 状态，确保不覆盖已有数据
                current_paper = await PaperRepository.get_paper_by_id(session, paper_id)
                if not current_paper:
                    logger.error(f"Paper {paper_id} record lost during processing")
                    return False

                # 智能更新策略:
                # 1. Title: 只有当原先标题为空，或者是通用文件名（如以 .pdf 结尾）时，才用解析出的标题覆盖
                update_title = None
                if title and (not current_paper.title or current_paper.title.lower().endswith(".pdf") or current_paper.title == current_paper.file_key.split("/")[-1]):
                    update_title = title
                
                # 2. Authors: 只有原作者列表为空时，才更新
                update_authors = None
                if authors and (not current_paper.authors or current_paper.authors == [""] or current_paper.authors == []):
                    update_authors = authors
                
                # 3. Summary: 只有原摘要为空时，才更新
                update_summary = None
                if summary and not current_paper.summary:
                ...
                await PaperRepository.update_paper_metadata(
                    ...
                    published_at=published_at if not current_paper.published_at else None,
                    source=metadata.get("source") if not current_paper.source or current_paper.source == "upload" else None,
                    source_id=metadata.get("source_id") if not current_paper.source_ref else None
                )
                await PaperRepository.update_paper_status(session, paper_id, PaperStatus.PARSED)
    ...
    async def vectorize(self, paper_id: UUID, job_id: UUID, redis: Any = None) -> bool:
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                effective_config = await setting_service.get_effective_model_config(paper.user_id, 'embedding')
                
                # 构造 embedding_config (兼容 _generate_embeddings 期望的字段名)
                embedding_config = {
                    "embedding_provider": effective_config.get("provider"),
                    "embedding_model": effective_config.get("model_name"),
                    "embedding_api_key": effective_config.get("api_key"),
                    "embedding_base_url": effective_config.get("base_url")
                }
                
                # 安全检查: 如果 API Key 为空或包含掩码, 则发出警告
                if effective_config.get("provider") in ['openai', 'siliconflow']:
                    if not effective_config.get("api_key"):
                        logger.warning(f"Vectorize for paper {paper_id}: Embedding API Key is EMPTY")
                    elif "****" in effective_config.get("api_key"):
                        logger.error(f"Vectorize for paper {paper_id}: Embedding API Key is MASKED, this will fail")
                
                logger.info(f"调度嵌入模型: {embedding_config.get('embedding_model')} (Provider: {embedding_config.get('embedding_provider')}, BaseURL: {embedding_config.get('embedding_base_url')})")
    ...
    async def summary(self, paper_id: UUID, job_id: UUID, redis: Any = None) -> bool:
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                # 2. 获取生效的 LLM 配置 (自动处理 用户设置 > 系统全局配置 的回退逻辑)
                from service.setting.setting_service import SettingService
                setting_service = SettingService(session)
                llm_config = await setting_service.get_effective_model_config(paper.user_id, 'summary')
                
                # 兼容性适配: SummaryAgent 可能期望 model_provider 字段
                if "provider" in llm_config and "model_provider" not in llm_config:
                    llm_config["model_provider"] = llm_config["provider"]

                # 安全检查: 如果 API Key 为空或包含掩码, 则发出警告
                if not llm_config.get("api_key"):
                    logger.warning(f"Summary Job for paper {paper_id}: API Key is EMPTY (Using config type: summary)")
                elif "****" in llm_config.get("api_key"):
    ...
    async def mind_map(self, paper_id: UUID, job_id: UUID, redis: Any = None) -> bool:
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                from service.setting.setting_service import SettingService
                setting_service = SettingService(session)
                llm_config = await setting_service.get_effective_model_config(paper.user_id, 'mind_map')
                
                # 兼容性适配: MindMapAgent 可能期望 model_provider 字段
                if "provider" in llm_config and "model_provider" not in llm_config:
                    llm_config["model_provider"] = llm_config["provider"]
                
                # 覆盖 temperature 为 0.3 (脑图生成需要较低的随机性)
                llm_config["temperature"] = 0.3

                # 安全检查: 如果 API Key 为空或包含掩码, 则发出警告
                if not llm_config.get("api_key"):
                    logger.warning(f"MindMap Job for paper {paper_id}: API Key is EMPTY (Using config type: mind_map)")
                elif "****" in llm_config.get("api_key"):
                    logger.error(f"MindMap Job for paper {paper_id}: API Key is MASKED, this will fail LLM call")
    ...
    async def _trigger_next_tasks(self, paper_id: UUID, user_id: UUID):
        """触发后续任务: Vectorize, Summary, MindMap (带去重检查)"""
        from service.reader.job_service import JobService
        from controller.api.reader.schema import JobCreateRequest
        from service.setting.setting_service import SettingService
        ...
        async with async_session_factory() as session:
            ...
            # 辅助函数: 检查是否已有相同类型的活跃 job (非 failed)
            async def _has_active_job(job_type: str) -> bool:
                stmt = select(Job).where(
                    and_(
                        Job.paper_id == paper_id,
                        Job.type == job_type,
                        Job.status.notin_(["failed", "cancelled"])
                    )
                )
                result = await session.execute(stmt)
                return result.scalar_one_or_none() is not None

            # Get unified model config to check if embedding is enabled
            try:
                effective_config = await setting_service.get_effective_model_config(user_id, 'embedding')
                embedding_provider = effective_config.get("provider", "local")
            except Exception as e:
                logger.warning(f"Failed to get effective settings, using default: {e}")
                embedding_provider = settings.embedding_type
            
            # 1. Vectorize (Only if not disabled and no active job exists)
            if embedding_provider != "none":
                if not await _has_active_job("vectorize"):
                    await job_service.create_job(paper_id, JobCreateRequest(job_type="vectorize"), user_id)
                else:
                    logger.info(f"Skipping duplicate vectorize job for paper {paper_id}")
            else:
                logger.info(f"Skipping vectorize job for paper {paper_id} (Provider: {embedding_provider})")
            
            # 2. Summary
            if not await _has_active_job("summary"):
                await job_service.create_job(paper_id, JobCreateRequest(job_type="summary"), user_id)
            else:
                logger.info(f"Skipping duplicate summary job for paper {paper_id}")
            
            # 3. MindMap
            if not await _has_active_job("mind_map"):
                await job_service.create_job(paper_id, JobCreateRequest(job_type="mind_map"), user_id)
            else:
                logger.info(f"Skipping duplicate mind_map job for paper {paper_id}")
    ...
    async def _parse_pdf(self, file_path: Path, parser_type: str = "auto") -> Optional[str]:
        """
        解析PDF文件
        """
        try:
            ...
            # 使用PDF解析器提取文本
            text_content = await extract_pdf_text(file_path, parser_type=parser_type)
            logger.info(f"PDF解析完成，文本长度: {len(text_content) if text_content else 0}")
            return text_content
        except Exception as e:
    ...
    async def _extract_metadata(
        ...
        try:
            ...
            # 清理标题：去除两端空格，如果为空则设为 None
            title = parse_result.title.strip() if parse_result.title else None
            if not title:
                title = None
            
            # 清理作者：过滤掉空字符串
            authors = [a.strip() for a in (parse_result.authors or []) if a and a.strip()]

            metadata = {
                "title": title or file_path.stem,
                "authors": authors,
                "abstract": parse_result.abstract.strip() if parse_result.abstract else None,
