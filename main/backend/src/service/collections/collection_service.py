...
from base.pg.service import CollectionRepository, PaperRepository, SessionDep, UserRepository, JobRepository
...
class CollectionService:
    ...
    async def get_collection_details(self, collection_id: UUID, user_id: UUID) -> CollectionDetailDTO:
        ...
        items = []
        for p in papers:
            # 尝试获取该论文的最相关主任务 (process_pdf 或 vectorize)
            try:
                job = await JobRepository.get_primary_job_by_paper_id(self.session, p.id)
                job_id = str(job.id) if job else None
            except Exception as e:
                logger.error(f"Error getting latest job for paper {p.id}: {e}")
                job_id = None
