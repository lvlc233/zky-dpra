'''
开发者: BackendAgent
当前版本: v1.3_marker_2x_fallback
创建时间: 2026年01月08日 15:00
更新时间: 2026年04月06日 11:00
更新记录:
    [2026年04月06日 11:00:v1.3_marker_2x_fallback:适配 Marker 2.x API 并增加自动降级容错机制]
...
# MARKER_AVAILABLE and PYMUPDF_AVAILABLE will be checked lazily
MARKER_AVAILABLE = None
PYMUPDF_AVAILABLE = None

def check_marker():
    global MARKER_AVAILABLE
    if MARKER_AVAILABLE is not None:
...
def check_pymupdf():
    ...
    if PYMUPDF_AVAILABLE is not None:
...
class MarkerPDFParser(BasePDFParser):
    ...
    def __init__(self):
        ...
        logger.info("MarkerPDFParser初始化完成 (模型将延迟加载)")
    ...
    def _ensure_models_loaded(self):
        ...
        try:
            ...
            logger.info("正在加载 Marker 模型 (v2.x API)...")
            
            # 使用 ConfigParser 正确初始化模型
            config_parser = ConfigParser({"output_format": "markdown"})
            artifact_dict = create_model_dict()
            
            # 兼容性修复：如果 artifact_dict 还是旧版格式或不符合预期，
            # PdfConverter 可能在内部报错。
            ...
            logger.info("Marker 模型加载完成")
        except Exception as e:
            logger.error(f"Marker 模型加载失败: {e}")
            # 注意: 这里不直接抛出异常，让 parse 方法去捕获并允许外部 fallback
            self.converter = None
            raise RuntimeError(f"Marker 模型加载失败: {e}")
...
class PyMuPDFParser(BasePDFParser):
    ...
    def __init__(self):
        self.fitz_available = check_pymupdf()
    ...
    def _parse_sync(self, file_path: Path) -> PDFParseResult:
        ...
        fitz_lib = self._get_fitz()
        doc = fitz_lib.open(str(file_path))
    ...
    def _extract_authors_from_text(self, text: str) -> List[str]:
        ...
        for pattern in patterns:
            match = re.search(pattern, text[:1500], re.IGNORECASE | re.MULTILINE)
            if match:
                author_text = match.group(1).strip()
                # 分割多个作者
                if ',' in author_text:
                    temp_authors = [name.strip() for name in author_text.split(',')]
                elif ' and ' in author_text:
                    temp_authors = [name.strip() for name in author_text.split(' and ')]
                else:
                    temp_authors = [author_text]
                
                # 过滤掉空字符串和无效内容
                authors = [a for a in temp_authors if a and len(a) > 1 and not re.search(r'[@#$]', a)]
                break
...
async def parse_pdf(file_path: Path, parser_type: str = "auto") -> PDFParseResult:
    ...
    try:
        ...
        if parser_type == "auto" and not isinstance(parser, PyMuPDFParser):
            logger.warning(f"主解析器失败，尝试由于主解析器失败而降级到 PyMuPDF: {e}")
            try:
                fallback_parser = PyMuPDFParser()
                return await fallback_parser.parse(file_path)
            except Exception as fe:
                logger.error(f"降级解析也失败了: {fe}")
                raise fe
        raise e
...
async def extract_pdf_text(file_path: Path, parser_type: str = "auto") -> str:
    ...
    try:
        ...
        if parser_type == "auto" and not isinstance(parser, PyMuPDFParser):
             logger.warning(f"主解析器提取文本失败，尝试由于主解析器失败而降级到 PyMuPDF: {e}")
             try:
                 fallback_parser = PyMuPDFParser()
                 return await fallback_parser.extract_text(file_path)
             except Exception as fe:
                 logger.error(f"降级提取文本也失败了: {fe}")
                 raise fe
        raise e