...
class User(SQLModel, table=True):
    ...
    is_admin: bool = Field(
        default=False,
        sa_column_kwargs={"comment": "是否为系统管理员"}
    )
...
class SearchApiConfig(SQLModel, table=True):
    """
    全局搜索源配置表 (SearchApiConfig)
    存储系统级别的外部搜索 API 配置（如 arXiv, Semantic Scholar 等）。
    """
    __tablename__ = "search_api_configs"
    __table_args__ = {"comment": "全局搜索源配置表: 存储 API 密钥、权重和激活状态"}

    id: UUID = Field(
        default_factory=uuid4,
        primary_key=True,
        sa_type=PGUUID(as_uuid=True),
        sa_column_kwargs={"comment": "配置记录唯一标识"}
    )
    api_name: str = Field(
        unique=True,
        index=True,
        sa_column_kwargs={"comment": "接口平台名称(如 arXiv, Semantic Scholar)"}
    )
    api_key: str = Field(
        default="",
        sa_column_kwargs={"comment": "API 密钥/凭据"}
    )
    weight: int = Field(
        default=0,
        sa_column_kwargs={"comment": "搜索权重/优先级(权重越高越优先使用)"}
    )
    is_active: bool = Field(
        default=True,
        sa_column_kwargs={"comment": "是否激活该数据源"}
    )
    created_at: datetime = Field(
        default_factory=datetime.now,
        sa_column_kwargs={"comment": "创建时间"}
...
class SystemModelConfig(SQLModel, table=True):
    """
    系统模型配置表 (SystemModelConfig)
    存储全系统默认的模型供应商、名称、接口地址、密钥、系统提示词及参数。
    """
    __tablename__ = "system_model_configs"
    __table_args__ = {"comment": "全系统模型配置表: 存储默认 LLM 供应商及参数"}

    id: UUID = Field(
        default_factory=uuid4,
        primary_key=True,
        sa_type=PGUUID(as_uuid=True),
        sa_column_kwargs={"comment": "配置唯一标识"}
    )
    type: str = Field(
        index=True,
        unique=True,
        sa_column_kwargs={"comment": "配置类型 (chat, summary, mind_map)"}
    )
    provider: str = Field(
        default="openai",
        sa_column_kwargs={"comment": "模型供应商 (openai, siliconflow, ollama, etc.)"}
    )
    model_name: str = Field(
        default="gpt-4o-mini",
        sa_column_kwargs={"comment": "模型名称 (Model Name)"}
    )
    base_url: Optional[str] = Field(
        default=None,
        sa_column_kwargs={"comment": "接口基础地址"}
    )
    api_key: str = Field(
        default="",
        sa_column_kwargs={"comment": "LLM API Key"}
    )
    system_prompt: str = Field(
        default="",
        sa_column_kwargs={"comment": "系统提示词 (System Prompt)"}
    )
    temperature: float = Field(
        default=0.7,
        sa_column_kwargs={"comment": "模型采样温度"}
    )
    max_tokens: int = Field(
        default=4096,
        sa_column_kwargs={"comment": "最大生成长度"}
    )
    is_active: bool = Field(
        default=True,
        sa_column_kwargs={"comment": "是否激活该配置"}
    )
    created_at: datetime = Field(
        default_factory=datetime.now,
        sa_column_kwargs={"comment": "创建时间"}
