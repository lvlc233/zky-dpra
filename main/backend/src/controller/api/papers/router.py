...
def calculate_progress(status_value: PaperStatus) -> int:
    progress_map = {
        ...
        PaperStatus.PARSING: 20,
        PaperStatus.PARSED: 40,
        PaperStatus.PROCESSING: 60,
...
async def get_paper_status(
    ...
    return Response.success(data=PaperStatusResponse(
        ...
        progress=calculate_progress(paper.analysis_status),
...
async def list_user_papers(
    ...
    for p in papers:
        logger.info(f"Paper: {p.id} - {p.title} - {p.analysis_status}")
    return Response.success(data=[
        PaperStatusResponse(
            ...
            authors=p.authors,
            abstract=p.abstract,
            progress=calculate_progress(p.analysis_status),
