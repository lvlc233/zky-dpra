...
# --- Search API Configs (Admin) ---

@router.get("/search-api-configs", response_model=Response[SearchApiConfigListResponse])
async def get_search_api_configs(
    current_user: CurrentUserDep,
    service: SettingServiceDep,
):
    """获取所有搜索 API 配置 (管理员)"""
    if current_user.email != 'admin@drap.com':
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="权限不足")
    
    configs = await service.get_all_search_api_configs(current_user.id)
    return Response.success(data=SearchApiConfigListResponse(configs=configs))


@router.post("/search-api-configs", response_model=Response[SearchApiConfigInfo])
async def create_or_update_search_api_config(
    data: SearchApiConfigUpdate,
    current_user: CurrentUserDep,
    service: SettingServiceDep,
):
    """创建或更新搜索 API 配置 (管理员)"""
    ...
    config = await service.update_search_api_config(
...
@router.delete("/search-api-configs/{api_name}", response_model=Response[bool])
async def delete_search_api_config(
    api_name: str,
    current_user: CurrentUserDep,
    service: SettingServiceDep,
):
    ...
    success = await service.delete_search_api_config(current_user.id, api_name)
    return Response.success(data=success)


@router.get("/system/stats", response_model=Response[SystemStatsResponse])
async def get_system_stats(
    ...
    return Response.success(data=SystemStatsResponse(stats=stats))
